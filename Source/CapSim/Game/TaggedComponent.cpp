...
UTaggedComponent::UTaggedComponent(const FObjectInitializer& ObjectInitializer) :
	...
	Color(1, 1, 1, 1)
{
	FString MaterialPath = TEXT("Material'/Game/PostProcessingMaterials/AnnotationColor.AnnotationColor'");
	...
	UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent constructor called."));
}
...
void UTaggedComponent::OnRegister()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent OnRegister called. Setting color: %s"), *(Color.ToString()));
...
void UTaggedComponent::SetColor(FLinearColor NewColor)
{
	...
	if (IsValid(TaggedMID))
	{
		...
		FLinearColor a;
		TaggedMID->GetVectorParameterValue(TEXT("AnnotationColor"), a);
		UE_LOG(LogTemp, Warning, TEXT("TaggedMID: %s"), *(a.ToString()));
	}
}
...
FPrimitiveSceneProxy* UTaggedComponent::CreateSceneProxy()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid parent component"));
		return NULL;
	}
	...
	*/
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent CreateSceneProxy HierarchicalInstancedStaticMeshComponent"));
		return CreateSceneProxy(HierarchicalInstancedStaticMeshComponent);
	}
	else if (InstancedStaticMeshComponent)
	{
		UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent CreateSceneProxy InstancedStaticMeshComponent"));
		return CreateSceneProxy(InstancedStaticMeshComponent);
	}
	*/
	if (IsValid(StaticMeshComponent))
	{
		UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent CreateSceneProxy StaticMeshComponent"));

		return CreateSceneProxy(StaticMeshComponent);
	}
	else if (IsValid(SkeletalMeshComponent))
	{
		UE_LOG(LogTemp, Warning, TEXT("LogNavid: TaggedComponent CreateSceneProxy SkeletalMeshComponent"));
		bSkeletalMesh = true;
		return CreateSceneProxy(SkeletalMeshComponent);
	}


	UE_LOG(LogTemp, Error, TEXT("Unknown type of parent component: %s"), *ParentComponent->GetClass()->GetName());
...
FPrimitiveSceneProxy* UTaggedComponent::CreateSceneProxy(UStaticMeshComponent* StaticMeshComponent)
{
	...
	if (StaticMesh == NULL)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create scene proxy for static mesh component (because static mesh is null): %s"), *StaticMeshComponent->GetReadableName());
		return NULL;
	}

	if (StaticMesh->RenderData == NULL)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create scene proxy for static mesh component (because render data is null): %s"), *StaticMeshComponent->GetReadableName());
		return NULL;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create scene proxy for static mesh component (because num LOD resources is 0): %s"), *StaticMeshComponent->GetReadableName());
		return NULL;
	}
...
FPrimitiveSceneProxy* UTaggedComponent::CreateSceneProxy(USkeletalMeshComponent* SkeletalMeshComponent)
{
		...
		SkeletalMeshComponent->MeshObject)
	{
		...
		int32 MaxSupportedNumBones = SkeletalMeshComponent->MeshObject->IsCPUSkinned() ? MAX_int32 : 66536;
